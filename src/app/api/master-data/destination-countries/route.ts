...
// GET /api/master-data/destination-countries
export const GET = requireAuth(async (request: any) => {
  try {
    ...
    response.headers.set('ETag', `destination-countries-${Date.now()}`);
    
    return response;
  } catch (error) {
    console.error('Error fetching destination countries:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to fetch destination countries' },
      { status: 500 }
    );
  }
});