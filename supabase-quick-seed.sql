-- Quick Atlas Platform Seeding Script for Supabase
...
-- Insert Countries and Platforms
...
-- Insert sample agents for each branch
WITH agent_data AS (
  SELECT 
...
-- Create Admin User: admin@atlas.com / password123
INSERT INTO users (id, email, password_hash, role, name, branch_id, is_active, created_at, updated_at)
SELECT 
  uuid_generate_v4(),
  'admin@atlas.com',
  '$2a$12$JoxeVBbG6m4eavRhWj9xDuBwNdOi50PNGkfmicE0C03ge/KHck.Lm',
  'ADMIN',
  'Atlas Administrator',
  b.id,
  true,
...
SELECT 
  ...
  (SELECT COUNT(*) FROM destination_countries) as destinations,
  (SELECT COUNT(*) FROM target_countries) as targets,
  (SELECT COUNT(*) FROM advertising_platforms) as platforms,
