...
-- Optimized for the most common query patterns identified in performance audit
...
CREATE INDEX IF NOT EXISTS idx_advertising_platforms_name_lower 
ON advertising_platforms(LOWER(name));
...
CREATE INDEX IF NOT EXISTS idx_campaign_details_date_platform_amount 
ON campaign_details(DATE(created_at), platform_id, amount);

CREATE INDEX IF NOT EXISTS idx_sales_country_data_date_deals_quality 
ON sales_country_data(DATE(created_at), deals_closed, quality_rating);

-- Audit and monitoring indexes
CREATE INDEX IF NOT EXISTS idx_media_reports_user_date 
ON media_reports(created_by, DATE(created_at));

CREATE INDEX IF NOT EXISTS idx_sales_reports_user_date 
ON sales_reports(created_by, DATE(created_at));